...
public class Music {
	...
	//Sets the JDA that will be used to find voice channels and guilds, 
	//but through the Discord class instead of using JDA.
	public static void setDiscord(Discord d) {
		jda = d.getJDA();
	}
	...
	public static void joinChannel(VoiceChannel vc) {
		audioManager = jda.getVoiceChannelById(vc.getId()).getGuild().getAudioManager();
		audioManager.openAudioConnection(jda.getVoiceChannelById(vc.getId()));
